...
class SpatialMemoryAttention(nn.Module):
    ...
    def forward(self, H_temp: torch.Tensor, A_static: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        K = self.W_K(self.PHI_m)
        V = self.W_V(self.PHI_m)
        Psi = self.W_PSI(self.PSI_m)

        phi_K = positive_feature_map(K)
        KtV = torch.einsum("me,md->ed", phi_K, V)

        A_bias = self.leaky(torch.matmul(self.node_E1, self.node_E2.t()))


        H_bt = H_temp.contiguous().view(B * T, N, d)
        Q = self.W_Q(H_bt)
        phi_Q = positive_feature_map(Q)


        QK = torch.einsum("bne,me->bnm", phi_Q, phi_K) / math.sqrt(self.E)
        A_dyn_qm = torch.softmax(QK, dim=-1)


        main_att = torch.einsum("bne,ed->bnd", phi_Q, KtV)
        mem_val = torch.einsum("bnm,md->bnd", A_dyn_qm, V)

        A_bias_exp = A_bias.unsqueeze(0).expand(B * T, -1, -1)
        bias_val = torch.bmm(A_bias_exp, mem_val)

        att_out = main_att + bias_val

        psi_mod = torch.einsum("bnm,md->bnd", A_dyn_qm, Psi)
        ...
        mem_val_bt = mem_val.view(B, T, N, d).mean(dim=1)
...
class SoftDTWSemanticAttention(nn.Module):
    ...
    def _softdtw_matrix(self, X_sp: torch.Tensor) -> torch.Tensor:
        ...
        for b in range(B):
            X_b = X_sp[b].permute(1, 0, 2).contiguous()
            A = X_b[I]
            Bseq = X_b[J]

            A2 = A.unsqueeze(2)
            B2 = Bseq.unsqueeze(1)
            D = torch.norm(A2 - B2, dim=-1)
    ...
    def forward(self, H_temp: torch.Tensor, X_sp: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        C_soft = self._softdtw_matrix(X_sp)
        M_sem = self._topl_mask(C_soft, self.top_l).detach()
        ...
        scores = torch.einsum("bnd,bmd->bnm", Q, K) / math.sqrt(d)
        M_rep = M_sem.unsqueeze(1).repeat(1, T, 1, 1).contiguous().view(B * T, N, N)
        scores = scores.masked_fill(M_rep < 1.0, float("-inf"))
        A_sem = torch.softmax(scores, dim=-1)

        H_sem_bt = torch.bmm(A_sem, V)
        H_sem = H_sem_bt.view(B, T, N, d)
        H_sem = self.norm(H_sem)
        A_sem_out = A_sem.view(B, T, N, N).mean(dim=1)
        return H_sem, A_sem_out
